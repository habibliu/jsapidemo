 ...
 *	
 */
public class QueryParkOut extends APIService {

	@Override
	protected String buildRequestParam() {
		Properties prop = ConfigHelper.getProperties(baseDir + "/park/flow/queryparkout");
	...
	protected void extractResult(CloseableHttpResponse response)
		...
		if (statusCode == HttpStatus.SC_OK) {
			...
			if (resultCode == 0) {
				System.out.println("QueryParkOut-SUCCESS:成功!\n信息：" + dataItems.toString());
			} else if (resultCode == 2) {
				System.out.println("QueryParkOut:失败!\t参数不正确");
			} else {
				System.out.println("QueryParkOut:失败!\t" + "resultCode:" + resultCode);
			}
		} else {
			System.out.println("QueryParkOut-ERROR:执行失败！" + "\tstatusCode:" + statusCode);
		}
	}
