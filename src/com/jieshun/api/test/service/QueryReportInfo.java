 ...
 *	
 */
public class QueryReportInfo extends APIService {

	@Override
	protected String buildRequestParam() {
		Properties prop = ConfigHelper.getProperties(baseDir + "/estate/report/queryreportinfo");
	...
	protected void extractResult(CloseableHttpResponse response)
		...
		if (statusCode == HttpStatus.SC_OK) {
			...
			if (resultCode == 0) {
				System.out.println("QueryReportInfo-SUCCESS:成功!\n信息：" + dataItems.toString());
			} else if (resultCode == 2) {
				System.out.println("QueryReportInfo:失败!\t参数不正确");
			} else if (resultCode == 3102) {
				System.out.println("QueryReportInfo:失败!\t人员不存在");
			}else {
				System.out.println("QueryReportInfo:失败!\t" + "resultCode:" + resultCode);
			}
		} else {
			System.out.println("QueryReportInfo-ERROR:执行失败！" + "\tstatusCode:" + statusCode);
		}
	}
