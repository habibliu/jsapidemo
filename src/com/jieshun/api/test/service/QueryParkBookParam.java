...
public class QueryParkBookParam extends APIService {

	/**
	 * "serviceId":"3c.park.queryparkbookparam",
		"requestType":"DATA",
		"attributes":{
	...
	protected String buildRequestParam() {
		Properties prop = ConfigHelper.getProperties(baseDir+"park/resrve/queryparkbookparam");
	...
	protected void extractResult(CloseableHttpResponse response)
		...
		if (statusCode == HttpStatus.SC_OK) {
			...
			if (resultCode == 0) {
				...
				System.out.println("queryparkbookparam-SUCCESS:获取车位预定列表成功!\n\t返回信息如下："+dataItems.toString());
			} else {
				System.out.println("queryparkbookparam-SUCCESS:获取车位预定列表失败!\n\t未知错误！");
			}
		} else {
			System.out.println("queryparkbookparam-ERROR:执行失败！" + "\tstatusCode:" + statusCode);
		}
	}
